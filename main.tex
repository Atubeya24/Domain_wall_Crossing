...
\end{align*}
特に、$e_{1}=e_{2}$の時の直感を述べるのは重要である。この場合、写像されたDomain-wall系は、以下のような描像となる。
\begin{resultbox}{$e_{1}=e_{2}$かつdiagonal部分群と双対なDomain-wall系}
    Domain-wall系は、su(N)の部分が連続で、直交補空間がD5-likeな境界条件となる。
\end{resultbox}
\end{enumerate}
...
\begin{resultbox}{1-2 Domain-wall上($x^{2}\geq0,x^{3}=0$)の境界条件}
    ...
    \end{align*}
    この場合、Domain-wall上の境界条件は以下のようになる。
    \begin{enumerate}
        \item $h_{21}$成分の境界条件
        \begin{align*}
            &if\,\Phi_{D}(x^{3}\geq 0)\in h_{21}\\
            & \implies \Phi_{D}(x^{3}=+0)=i_{21}(\Phi(x^{3}=-0))\\
        \end{align*}
        \item 直交補空間上の境界条件
        \begin{align*}
            (\varepsilon_{abc}D_{3}\vec{X}_{a}^{\perp_{21}}-[X_{b}^{\perp_{21}},X_{c}^{\perp_{21}}]^{\perp_{21}})|_{x^{3}=+0,x^{2}\geq 0}=0\\
            \vec{Y}^{\perp_{21}}|_{x^{3}=+0,x^{2}\geq 0}=0\\
            B_{1}\Psi^{\perp_{21}}|_{x^{3}=+0,x^{2}\geq 0}=\Psi^{\perp_{21}}
        \end{align*}
        ただし、$\Phi^{\perp}$の定義は、以下の通り。
        \begin{align*}
            \Phi^{\perp_{21}}&\in h_{21}^{\perp}
        \end{align*}
    \end{enumerate}    
\end{resultbox}
\begin{resultbox}{2-3 Domain-wall上($x^{2}=0,x^{3}\leq0$)の境界条件}
    まず、su(N)内に以下のような部分群が自然に存在。
    \begin{align*}
    ...
    \begin{enumerate}
        \item $h_{23}$成分の境界条件
        \begin{align*}
            &if\,\Phi_{D}(x^{2}\geq 0)\in h_{32}\\
            & \implies \Phi_{D}(x^{2}=+0)=i_{32}(\Phi(x^{2}=-0))
        \end{align*}
        \item 直交補空間上の境界条件
        \begin{align*}
...
\begin{resultbox}{3-4 Domain-wall上($x^{2}\leq0,x^{3}=0$)の境界条件}
    ...
    \begin{enumerate}
        \item $h_{34}$成分の境界条件
        \begin{align*}
            &if\,\Phi_{D}(x^{3}\geq 0)\in h_{34}\\
            &\implies \Phi_{D}(x^{3}=+0)=i_{34}(\Phi(x^{3}=-0))
        \end{align*}
        \item 直交補空間上の境界条件
        \begin{align*}
            (\varepsilon_{abc}D_{3}\vec{X}_{a}^{\perp}-[X_{b}^{\perp_{34}},X_{c}^{\perp_{34}}]^{\perp_{34}})|_{x^{3}=+0,x^{2}\leq 0}=0\\
            \vec{Y}^{\perp_{34}}|_{x^{3}=+0,x^{2}\leq 0}=0\\
            B_{1}\Psi^{\perp_{34}}|_{x^{3}=+0,x^{2}\leq 0}=\Psi^{\perp_{34}}
        \end{align*}
        ただし、$\Phi^{\perp}$の定義は、以下の通り。
        \begin{align*}
            \Phi^{\perp_{34}}&\in h_{34}^{\perp}
        \end{align*}
    \end{enumerate}    
\end{resultbox}
\begin{resultbox}{4-1 Domain-wall上($x^{2}=0,x^{3}\geq0$)の境界条件}
    ...
    \begin{enumerate}
        \item $h_{41}$成分の境界条件
        \begin{align*}
            &if\,\Phi_{D}(x^{2}\geq 0)\in h_{41}\\
            &\implies \Phi_{D}(x^{2}=+0)=i_{41}(\Phi(x^{2}=-0))
        \end{align*}
        \item 直交補空間上の境界条件
        \begin{align*}
...
\end{center}
\end{resultbox}
これらのembeddingを考えると、$x^{2},x^{3}\leq0$のembeddingがなす部分空間、およびその直交補空間は、全て$x^{2},x^{3}\geq 0$の内部に包括されている。これは、embeddingと例えば連続条件といった議論がconsistencyであることを意味する\footnote{逆にembeddingがはみ出ていると、そこで連続とならない領域が生まれるため、inconsistencyとなる。}し、$x^{2},x^{3}\geq 0$の領域のみを考えると、その他の領域の境界条件が自動的に理解できることになる。故に、このembeddingを採用する場合に限り、$x^{2},x^{3}\geq 0$のみを考えれば良い。
\subsubsection{モジュライ空間($x^{2},x^{3}\geq 0$)}
上記のDomain-wall conditionを満たしつつ、Bulk上でのVEV-constraintを満たすような解について考える。まずVEV-constraintについて再掲する。ゲージ場のVEVが0である場合を考えると、これは今までのBulk上のVEVと同様の議論から、
\begin{resultbox}{Domain-wall系のBulk上のVEV}
    ...
    \end{align*}
    また、以下よりこれの特解として、以下のような形の解を考える。
    \begin{align*}
        X_{a}=X_{a}(x^{3}),\quad Y_{m}=Y_{m}(x^{2})
    \end{align*}
\end{resultbox}
また、連続条件を省いた、特記すべき境界条件を再掲すると、
\begin{resultbox}{D5-like Domain-wall condition}
    \begin{enumerate}
            \item $x^{3}=0$上のDomain-wall condition
            \begin{align*}
        ...
        \end{align*}
        ただし、リー代数の直和分解と、場の配位は以下の通りとなる。
        \begin{align*}
            su(N+k)&=h_{21}\oplus h_{21}^{\perp}\\
            \Phi^{\perp}&\in h_{21}^{\perp}
        \end{align*}
            \item $x^{2}=0$上の境界条件
        \begin{align*}
            X^{\perp'}|_{x^{2}=+0,x^{3}\geq 0}=0\\
            (\varepsilon_{lmn}D_{2}Y_{l}^{\perp'}-[Y_{m}^{\perp'},Y_{n}^{\perp'}]^{\perp'})|_{x^{2}=+0,x^{3}\geq 0}=0\\
            C_{2}\Psi^{\perp'}|_{x^{2}=+0,x^{3}\geq 0}=\Psi^{\perp'}
        \end{align*}
        ただし、リー代数の直和分解と、場の配位は以下の通りとなる。
        \begin{align*}
            su(N+k)=&h_{41}\oplus h_{41}^{\perp}\\
            \Phi^{\perp'}&\in h_{41}^{\perp}
        \end{align*}
        \end{enumerate}
\end{resultbox}
これらの式を、便宜上、2つの組にラベリングする。一方は、VEVの存在箇所を指定する式で、もう一方は、VEVの具体系を決定する式である。
\begin{defbox}{VEVの存在領域を決める式}
    \begin{align*}
    X_{a}=X_{a}(x^{3}),\quad Y_{m}&=Y_{m}(x^{2})\\
        \quad X^{\perp'}|_{x^{2}=+0,x^{3}\geq 0}&=0\\
        \vec{Y}^{\perp}|_{x^{3}=+0,x^{2}\geq 0}&=0\\
        [X_{a},Y_{m}]&=0
    \end{align*}
\end{defbox}
上式は、VEVの住んでいる領域を制限する式である。当然、どういった領域が制限されるかは、embeddingによって変化する。予め用意したembeddingを用いたとすると、上式を満たすVEVの特解として、以下のようなものが考えられる。\\
\\
また、VEVの具体系は、以下の方程式から導出できる。
\begin{defbox}{VEVの具体系を決める式}
    \begin{align*}
        0&=\bar{\epsilon}_{0}([X_{b},X_{c}]-\epsilon_{abc}D_{3}X_{a}B_{1})\\
    0&=\bar{\epsilon}_{0}([Y_{m},Y_{n}]-\epsilon_{lmn}D_{3}Y_{l}B_{2})\\
    0&=(\varepsilon_{abc}D_{3}\vec{X}_{a}^{\perp}-[X_{b}^{\perp},X_{c}^{\perp}]^{\perp})|_{x^{3}=+0,x^{2}\geq 0}\\
    0&=(\varepsilon_{lmn}D_{2}Y_{l}^{\perp'}-[Y_{m}^{\perp'},Y_{n}^{\perp'}]^{\perp'})|_{x^{2}=+0,x^{3}\geq 0}
    \end{align*}
\end{defbox}
これを満たす解として、定数解やPole解が考えられる。上記の解の存在領域として許される部分の中で、これらを満たす解を考えればいいだけなので、解の分類は非常にシンプルに行われる。
